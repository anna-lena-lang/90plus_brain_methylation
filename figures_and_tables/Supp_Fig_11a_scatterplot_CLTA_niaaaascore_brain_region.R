...
## DEN
neuron_promoters_DEN <- neuron_promoters_DEN[CLTA,]
...
neuron_promoters_DEN$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_DEN <- neuron_promoters_DEN  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_DEN$niaaaascore %<>% as.numeric

## CA1
neuron_promoters_CA1 <- neuron_promoters_CA1[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_CA1$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_CA1 <- neuron_promoters_CA1  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_CA1$niaaaascore %<>% as.numeric

## ERC
neuron_promoters_ERC <- neuron_promoters_ERC[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_ERC$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_ERC <- neuron_promoters_ERC  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_ERC$niaaaascore %<>% as.numeric

## MFG
neuron_promoters_MFG <- neuron_promoters_MFG[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_MFG$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_MFG <- neuron_promoters_MFG  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_MFG$niaaaascore %<>% as.numeric

## SN
neuron_promoters_SN <- neuron_promoters_SN[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_SN$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_SN <- neuron_promoters_SN  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_SN$niaaaascore %<>% as.numeric

## LOC
neuron_promoters_LOC <- neuron_promoters_LOC[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_LOC$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_LOC <- neuron_promoters_LOC  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_LOC$niaaaascore %<>% as.numeric

## CBL
neuron_promoters_CBL <- neuron_promoters_CBL[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_CBL$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_CBL <- neuron_promoters_CBL  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
neuron_promoters_CBL$niaaaascore %<>% as.numeric

## CG
neuron_promoters_CG <- neuron_promoters_CG[CLTA,]
...
raw_CLTA <- raw_results %>% filter(raw_results$external_gene_name == 'CLTA')
raw_CLTA <- raw_CLTA %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
neuron_promoters_CG$ensembl_gene_id <- raw_CLTA$ensembl_gene_id
neuron_promoters_CG <- neuron_promoters_CG  %>% as.data.frame() %>% left_join(raw_CLTA, by=  "ensembl_gene_id")
...
joined_df$celltype <- factor(joined_df$celltype, levels = c("CLTA MFG",
                                                            "CLTA CG",
                                                            "CLTA DG **",
                                                            "CLTA CA1",
                                                            "CLTA EC",
...
ggsave(
  paste0(baseDir,"/plots/final_figures/Supp_Fig_CLTA_niaaaascore_neuron_all_brain_regions.png"),
  p_PEN,
  width = 174,
  height = 60,
  units = 'mm',
