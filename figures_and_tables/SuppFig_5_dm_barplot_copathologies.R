## code to plot copathologies figure with overview barplot 
...
phenotypes <- c("braak_for_lb_orig", 'mvl_score', 'tdp43') 
...
dm_barplots_overview <- function(biotype = "promoters"){
  ...
  for (phenotype in phenotypes){
    ...
    for (celltype in celltypes){
      for (region in all_regions){
        ...
        filename <- c(paste(baseDir, paste("/dm_results/continuous/sig/sig", phenotype, celltype, biotype, region,sep="_"),".csv", sep=""))
  ...
  ggsave(
    paste0(baseDir,"/plots/final_figures/Supp_Fig_sig_copath_dm_barplot_overview.png"),
    p,
    width = 174,
    height = 160,
    dpi = 1200, 
