...
bulk_promoters <- read_rds("./cleaned_mvals/cleaned_mvals_episcore/niaaaascore/bulk/bulk/corrected_pvals/bulk_promoters_DG_niaaaascore_cleaned_mvals.rds")
...
astro_promoters <- read_rds("./cleaned_mvals/cleaned_mvals_episcore/niaaaascore/astro/astro/corrected_pvals/astro_promoters_DG_niaaaascore_cleaned_mvals.rds")
oligo_opc_promoters <- read_rds("./cleaned_mvals/cleaned_mvals_episcore/niaaaascore/oligo_opc/oligo_opc/corrected_pvals/oligo_opc_promoters_DG_niaaaascore_cleaned_mvals.rds")
endo_promoters <- read_rds("./cleaned_mvals/cleaned_mvals_episcore/niaaaascore/endo/endo/corrected_pvals/endo_promoters_DG_niaaaascore_cleaned_mvals.rds")
microglia_promoters <- read_rds("./cleaned_mvals/cleaned_mvals_episcore/niaaaascore/microglia/microglia/corrected_pvals/microglia_promoters_DG_niaaaascore_cleaned_mvals.rds")
...
bulk_promoters_PEN2_df <- tibble(sample_name = names(bulk_promoters_PEN2), Methylation = bulk_promoters_PEN2, celltype = "PEN-2 DG Bulk",sig_color= "not_sig") %>%
    left_join(targets)
raw_results <- read.csv(paste0("~/90plus/dm_results/continuous/raw/raw_niaaaascore_bulk_promoters_DG.csv"))
...
astro_promoters_PEN2_df <- tibble(sample_name = names(astro_promoters_PEN2), Methylation = astro_promoters_PEN2, celltype = "PEN-2 DG Astrocytes",sig_color= "not_sig") %>%
    left_join(targets)
raw_results <- read.csv(paste0("~/90plus/dm_results/continuous/raw/raw_niaaaascore_astro_promoters_DG.csv"))
raw_results <- merge(x = raw_results,y = genes_with_description, by = "ensembl_gene_id",all.x=TRUE)
raw_PEN2 <- raw_results %>% filter(raw_results$external_gene_name == 'PSENEN')
raw_PEN2 <- raw_PEN2 %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
...
oligo_opc_promoters_PEN2_df <- tibble(sample_name = names(oligo_opc_promoters_PEN2), Methylation = oligo_opc_promoters_PEN2, celltype = "PEN-2 DG Olig/OPCs",sig_color= "not_sig") %>%
    left_join(targets)
raw_results <- read.csv(paste0("~/90plus/dm_results/continuous/raw/raw_niaaaascore_oligo_opc_promoters_DG.csv"))
raw_results <- merge(x = raw_results,y = genes_with_description, by = "ensembl_gene_id",all.x=TRUE)
raw_PEN2 <- raw_results %>% filter(raw_results$external_gene_name == 'PSENEN')
raw_PEN2 <- raw_PEN2 %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
...
raw_results <- read.csv(paste0("~/90plus/dm_results/continuous/raw/raw_niaaaascore_endo_promoters_DG.csv"))
raw_results <- merge(x = raw_results,y = genes_with_description, by = "ensembl_gene_id",all.x=TRUE)
raw_PEN2 <- raw_results %>% filter(raw_results$external_gene_name == 'PSENEN')
raw_PEN2 <- raw_PEN2 %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
...
raw_results <- read.csv(paste0("~/90plus/dm_results/continuous/raw/raw_niaaaascore_microglia_promoters_DG.csv"))
raw_results <- merge(x = raw_results,y = genes_with_description, by = "ensembl_gene_id",all.x=TRUE)
raw_PEN2 <- raw_results %>% filter(raw_results$external_gene_name == 'PSENEN')
raw_PEN2 <- raw_PEN2 %>% dplyr::select("ensembl_gene_id", "adj.P.Val")
